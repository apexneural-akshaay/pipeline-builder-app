...
model_m = YOLO("D:/APEXNEURAL TRAIL/V/Vision-Project/visionai-platform/pipeline-builder-app/backend/models/yolo26n.pt")
TASK_m = "detect"
...
# ── event_sink (e) ──
EVENTS_DIR_e = "D:/APEXNEURAL TRAIL/V/Vision-Project/visionai-platform/pipeline-builder-app/backend/events"
...
while True:
    ...
    try:
        annotated_m = results_m[0].plot()
    except Exception:
        annotated_m = frame_in

    state["m"] = {
        "frame": frame_in,
        "annotated": annotated_m,
        "detections": dets_m,
        "task": TASK_m,
        "tracking": TRACKING_m,
    }
    ...
    display_frame_e = upstream.get("annotated") if isinstance(upstream, dict) and upstream.get("annotated") is not None else upstream["frame"]
    buffer_e.append(display_frame_e)
    ...
    if upstream["detections"]:
        ...
        if now - _last_fire_e >= COOLDOWN_S_e:
            ...
            if SAVE_SCREENSHOT_e:
                ...
                cv2.imwrite(shot_path, display_frame_e)
                event_record["screenshot"] = shot_path
            if SAVE_CLIP_e:
                h, w = display_frame_e.shape[:2]
