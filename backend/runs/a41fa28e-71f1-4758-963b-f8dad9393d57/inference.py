...
def _flush_pending_clips_node_3():
    ...
    _wait_for_transcodes_node_3(timeout_s=20.0)
    _reap_transcodes_node_3()
...
_pending_transcodes_node_3 = []  # list of (popen, tmp_path, final_path)


def _reap_transcodes_node_3():
    """Poll any in-flight transcode subprocesses; replace original mp4 when done."""
    still = []
    for popen, tmp, final in _pending_transcodes_node_3:
        rc = popen.poll()
        if rc is None:
        ...
        if rc == 0 and os.path.exists(tmp):
            try:
                ...
                print(f"event node_3: transcoded -> {final}")
            except Exception as e:
                print(f"event node_3: transcode replace failed: {e}")
        else:
            if os.path.exists(tmp):
                try: os.remove(tmp)
                except Exception: pass
            print(f"event node_3: transcode rc={rc} (keeping mp4v fallback)")
    _pending_transcodes_node_3[:] = still
...
def _transcode_to_h264_node_3(path):
    ...
    try:
        popen = subprocess.Popen(
            [_FFMPEG_EXE_node_3, "-y", "-i", path, "-c:v", "libx264", "-preset", "ultrafast",
             "-pix_fmt", "yuv420p", "-movflags", "+faststart", "-an", tmp],
            stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL,
        )
        _pending_transcodes_node_3.append((popen, tmp, path))
    except Exception as e:
        print(f"event node_3: transcode spawn failed ({e})")


def _wait_for_transcodes_node_3(timeout_s=30.0):
    """Block until all in-flight transcodes finish or the timeout elapses (exit-time helper)."""
    import time as _t
    deadline = _t.time() + timeout_s
    while _pending_transcodes_node_3 and _t.time() < deadline:
        _reap_transcodes_node_3()
        _t.sleep(0.2)
...
while not _SHOULD_EXIT:
    ...
    if upstream["detections"]:
        ...
        if now - _last_fire_node_3 >= COOLDOWN_S_node_3:
            ...
            if SAVE_CLIP_node_3:
                ...
                _pre_slice_node_3 = list(buffer_node_3)[-_pre_n_node_3:]
                pre_meta_node_3 = list(clip_meta_node_3)[-_pre_n_node_3:]
                for f in _pre_slice_node_3:
                    writer.write(f)
    ...
    if time.time() - _last_hb_e_node_3 >= 1.0:
        ...
        _last_hb_e_node_3 = time.time()
        _reap_transcodes_node_3()  # poll completed background transcodes
